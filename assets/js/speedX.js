$(document).ready(function(){
	...
	function gameLoop() {
		...
		if(down.indexOf(40) != -1 && paddleMargin < BOTTOM_WALL - LEFT_PADDLE_HEIGHT && !paused) {
		...
		if(down.indexOf(38) != -1 && paddleMargin > TOP_WALL && !paused) {
	...
	function winGame(winner) {
		var roundBallSpeed = Math.round(ballSpeed * 1000) / 1000;
		// var highScore = prompt("NEW HIGHSCORE!! Please type your name");

		if(previousScores.length === 5){
			previousScores.shift();
		}
		previousScores.push(roundBallSpeed);

		printBallSpeed(roundBallSpeed);
		// if(roundBallSpeed < 3){
		// 	highScore;
		// }
	...
	function printBallSpeed() {
		var ballSpeedScores = $('#ballSpeedScores');
		ballSpeedScores.empty();
		previousScores = previousScores.reverse();
		for(var i=0; i < previousScores.length; i++) {
			var template = '<li>'+previousScores[i]+'</li>';
			ballSpeedScores.append(template);
		}
	}
